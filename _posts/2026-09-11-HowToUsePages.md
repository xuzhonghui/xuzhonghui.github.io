...
当前博客会为**所有文章自动生成右侧大纲**。不需要在 Markdown 中手写“目录”，也不需要手动添加锚点。

只要使用 Markdown 标题，系统会自动提取 `##`、`###` 和 `####` 标题，并在桌面端正文右侧生成固定大纲。阅读正文时，大纲不会随页面向下滚动；点击任一大纲项会平滑跳转到对应段落。
